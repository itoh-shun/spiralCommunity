    ...
    const onSubmit: SubmitHandler<FormInputType> = async(data) => {
        if( data ) {
            try {
                await trigger( {
                    ...data,
                    tags: data.tags ? data.tags?.map( (tag: Tag) => tag?.name ) : [],
...
const UserInfo = () => {
    ...
    const { data } = useSWR<UserInfoType | null, Error, [ string ]>(
        [ `api/users/${ userId }` ],
        async([ url ]) => {
                ...
                <Grid size={ { xs: 12, lg: 4 } }>
                    <Card sx={ { mt: 2, position: 'relative' } }>
                        <IconButton size={ "small" }
                                    ...
                                    onClick={ async() => {
                                        user && await dialogs.open<UserInfoType, null>( EditProfile, user );
                                    } }><Edit/></IconButton>
                        <CardContent>
